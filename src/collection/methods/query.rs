...
use crate::database::cursors::query::{
    AddQueryCursorContinuationData, AddQueryCursorData, QueryCursor, QueryCursorPublicId,
    QueryCursorRef, QueryCursorRefCursor, QueryCursorRefEmpty,
};
use crate::messages::cursors::{
    AbortQueryCursorTask, AddQueryCursorContinuationTask, AddQueryCursorTask,
    DatabaseCollectionCursorsTask, FinishQueryCursorTask, FullyFinishQueryCursorTask,
    GetQueryCursorByPublicIdTask,
};
...
impl Collection {
            ...
            last_and_next_record_key @ Some(_) => {
                ...
                let id = async_sync_call(|sender| {
                    self.database_inner.add_cursors_task(
                        DatabaseCollectionCursorsTask::AddQueryCursor(AddQueryCursorTask {
    ...
    pub async fn read_query_cursor(
        ...
        let public_id = QueryCursorPublicId::from_b62(cursor_id.as_ref())
            .map_err(|_| CollectionMethodError::NoSuchCursor)?;

        let (inner_id, cursor) = async_sync_call(|sender| {
            self.database_inner.add_cursors_task(
                DatabaseCollectionCursorsTask::GetQueryCursorByPublicId(
                    GetQueryCursorByPublicIdTask {
                        collection_id: self.cursors_id,
                        public_id,
                        sender,
        ...
        let cursor = match cursor {
            QueryCursorRef::Cursor(cursor) => cursor,
            QueryCursorRef::Empty(QueryCursorRefEmpty { generation_id }) => {
                let _ = async_sync_call(|sender| {
                    self.database_inner.add_cursors_task(
                        DatabaseCollectionCursorsTask::FullyFinishQueryCursor(
                            FullyFinishQueryCursorTask {
                                collection_id: self.cursors_id,
                                inner_id,
                                sender,
                            },
        ...
        let QueryCursorRefCursor { cursor, is_current } = cursor;
        ...
        let cursor_public_id = match last_and_next_record_key {
            last_and_next_record_key @ Some(_) => {
                let cursor_public_id = async_sync_call(|sender| {
                    self.database_inner.add_cursors_task(
                        DatabaseCollectionCursorsTask::AddQueryCursorContinuation(
                            AddQueryCursorContinuationTask {
                                collection_id: self.cursors_id,
                                inner_id,
                                is_current,
            ...
            None => {
                let cursor_public_id = async_sync_call(|sender| {
                    self.database_inner.add_cursors_task(
                        DatabaseCollectionCursorsTask::FinishQueryCursor(FinishQueryCursorTask {
                            collection_id: self.cursors_id,
                            inner_id,
                            is_current,
                            sender,
    ...
    pub async fn abort_query_cursor(
        ...
        let public_id = QueryCursorPublicId::from_b62(&cursor_id)
            .map_err(|_| CollectionMethodError::NoSuchCursor)?;
        ...
        let _ = async_sync_call(|sender| {
            self.database_inner
                .add_cursors_task(DatabaseCollectionCursorsTask::AbortQueryCursor(
                    AbortQueryCursorTask {
                        collection_id: self.cursors_id,
                        public_id,
                        sender,
                    },
