...
impl AggregateTransform {
    ...
    fn on_generation_started(&mut self) -> HandlerResult<Self, HandlerContext> {
        ...
        let from_generation_id = from_generation_id_json.to_bytes().ok_or_else(|| {
            TransformError::Unspecified(format!("Invalid from_generation_id {from_generation_id_json:?}"))
        })?;
        let to_generation_id = to_generation_id_json.to_bytes().ok_or_else(|| {
            TransformError::Unspecified(format!("Invalid to_generation_id {to_generation_id_json:?}"))
        })?;
